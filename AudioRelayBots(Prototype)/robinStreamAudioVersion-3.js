...
client.on(Events.InteractionCreate, async (interaction) => {
  if (!interaction.isChatInputCommand()) return;
    ...
    if (interaction.commandName === "robin-3") {
      ...
      if (robinConnection) {
        await interaction.reply({ content: "ℹ️ Robin-3 is already capturing audio.", ephemeral: true });
        return;
      }
    ...
    await interaction.reply({ content: `🛰️ Robin-3 joined ${voiceChannel.name} and started spying mission.`, ephemeral: true });
    ...
    if (interaction.commandName === "stop-robin-3") {
      ...
      if (connection) {
        ...
        await interaction.reply({ content: "🛑 Robin-3 has left the mission.", ephemeral: true });
       // logChannel.send(`🛑 **Robin-3 has left** the voice channel in ${interaction.guild.name}.`);
      } else {
        await interaction.reply({ content: "⚠️ Robin-3 is not currently in a voice channel.", ephemeral: true });
      }
    }
});
